...
- (id)initWithDelegate:(id<DataRequestDelegate>)delegate
	...
	if(self) {
        ...
        if (!_requestUrl) {
            _requestUrl =  [[NSString stringWithFormat:@"%@/%@",Host_Url,[self getRequestUrl]] retain];
        }
        ...
        if (![NSString isBlankString:_cacheKey])
        {
...
- (void)processResult{
    ...
    _result = [[ITTRequestResult alloc] initWithCode:[NSString stringWithFormat:@"%@",resultDic[@"error_code"]]
                                         withMessage:resultDic[@"reason"]];
    
    if (![_result isSuccess]) {
        ITTDERROR(@"request[%@] failed with message %d",self,_result.isSuccess);
    }else {
...
//    NSString *trimmedString = [resultString stringByReplacingOccurrencesOfString:@"\\" withString:@""];
//    
//    NSString *trimmedString = [trimmedString stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"\""]];
    
    RELEASE_SAFELY(_resultString);
    
    
    
	_resultString = [resultString retain];

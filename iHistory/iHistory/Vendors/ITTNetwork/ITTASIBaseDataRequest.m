...
+(void)requestWithParameters:(NSDictionary *)params withIndicatorView:(UIView *)indiView onRequestStart:(void (^)(ITTBaseDataRequest *))onStartBlock onRequestFinished:(void (^)(ITTBaseDataRequest *))onFinishedBlock onRequestCanceled:(void (^)(ITTBaseDataRequest *))onCanceledBlock onRequestFailed:(void (^)(ITTBaseDataRequest *))onFailedBlock
{
    ...
    NSString *actionName = [self actionName];
    
    if (![NSString isBlankString:actionName])
    {
        NSMutableDictionary *newParamsDic = [NSMutableDictionary dictionary];
        [newParamsDic setObject:actionName forKey:@"action_name"];
        [newParamsDic setObject:params forKey:@"action_info"];
        
        NSMutableDictionary *params = [NSMutableDictionary dictionary];
        [params setObject:newParamsDic forKey:@"param"];
...
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setObject:@"8ccd88470cb7ee8752e05d4242cb37b1" forKey:@"key"];//key 服务器验证
    [params setObject:@"1.0" forKey:@"v"];//api版本号
    return params;
}
...
- (void)generateRequestWithUrl:(NSString*)url
 ...
 ///  url = [NSString stringWithFormat:@"%@e=ios&apiVersion=%@&",url,ApiVersion];
	...
	if ([self getRequestMethod] == ITTRequestMethodGet){
        ...
        for (NSString *key in [allParams allKeys]) {
            if ([allParams[key] isKindOfClass:[NSData class]]) {
                ...
                postBodySize += [[NSString stringWithFormat:@"%@",allParams[key]] lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
            }

//
//  DataKeyMacro.h
//  PMS
//
//  Created by ligh on 14/10/22.
//
//

#ifndef PMS_DataKeyMacro_h
#define PMS_DataKeyMacro_h

//HTTP key
#define HTTP_AlbuminfoResultKey     @"AlbuminfoResultKey"

//Data Key
#define Store_UserInfoKey           @"UserInfoKey"//登录的用户缓存key
#define EventListKey                @"EventListKey"
#define TODAYEventCachedtKey        @"TODAYEventCachedtKey"
#define EventDetailsDataKey         @"EventDetailsDataKey"

#define FristUseKey @"firstUseKey"

#endif
